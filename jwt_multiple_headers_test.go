...
func TestMultipleHeaderValues(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			headerValue:  "Bearer " + validToken + ", Bearer invalid",
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:         "valid token second in comma-separated list",
			headerValue:  "Bearer invalid, Bearer " + validToken,
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:         "valid token in middle of comma-separated list",
			headerValue:  "Bearer invalid1, Bearer " + validToken + ", Bearer invalid2",
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:        "all invalid tokens in comma-separated list",
			headerValue: "Bearer " + invalidToken + ", Bearer another.invalid.token",
			expectPass:  false,
		},
	}
...
func TestDuplicateHeaders(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			headerValues: []string{"Bearer " + validToken, "Bearer invalid"},
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:         "valid token in second header",
			headerValues: []string{"Bearer invalid", "Bearer " + validToken},
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:         "valid token in middle header",
			headerValues: []string{"Bearer invalid1", "Bearer " + validToken, "Bearer invalid2"},
			expectPass:   true,
			expectedName: "John Doe",
		},
		{
			name:         "all invalid tokens in duplicate headers",
			headerValues: []string{"Bearer " + invalidToken, "Bearer another.invalid.token"},
			expectPass:   false,
		},
	}
...
func TestCombinedMultipleHeadersAndValues(t *testing.T) {
	...
	req.Header["Authorization"] = []string{
		...
		"Bearer invalid3, Bearer " + validToken + ", Bearer invalid4",
	}
